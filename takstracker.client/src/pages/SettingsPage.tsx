...
const SettingsPage = () => {
  ...
  const [roleMenuUserId, setRoleMenuUserId] = React.useState<string | null>(
    null,
  );
  const [passwordUser, setPasswordUser] = React.useState<UserProfileDto | null>(
    null,
  );
  ...
  React.useEffect(() => {
    const loadUsers = async () => {
      ...
      try {
        ...
        setUsersError(
          status === 401
            ? "Zaloguj się, aby zarządzać użytkownikami."
                    ...
                    <div className="user-card-header">
                      ...
                      <div className="role-picker">
                        {user.userId !== profile.userId && (
                          <span className="role-picker-label">ZMIEŃ ROLĘ</span>
                        )}
                        <button
                          type="button"
        ...
        >
          ...
          >
            ...
            <form className="form" onSubmit={(event) => event.preventDefault()}>
              ...
              {passwordMessage && (
                <div className="banner">{passwordMessage}</div>
              )}
              {modalError && (
                <div className="banner modal-error">{modalError}</div>
              )}
