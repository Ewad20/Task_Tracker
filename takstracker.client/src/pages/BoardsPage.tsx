...
import type { ApiError, CreateTaskRequest, ProjectDto, TaskDto, UserProfileDto } from '../api/types'
...
const BoardsPage = ({ selectedProjectId, selectedProject, currentUser }: BoardsPageProps) => {
  ...
  const loadData = () =>
    Promise.all([
      ...
      .catch((err: unknown) => {
        ...
        setError(status === 401 ? 'Zaloguj się, aby pobrać zadania.' : 'Błąd pobierania zadań.')
      })
  ...
  const getProjectAssigneeOptions = (projectId: string) => {
    ...
    if (!isAdmin) {
      return currentUser && memberIds.has(currentUser.userId) ? [currentUser] : []
    }
  ...
  const handleSubmit = async () => {
    ...
    const assigneeId = isAdmin ? form.assigneeId : currentUser?.userId ?? form.assigneeId
  ...
  const handleDelete = async (task: TaskDto) => {
    ...
    try {
      ...
      const status = (err as ApiError)?.status
      setError(status === 401 ? 'Zaloguj się, aby usunąć zadanie.' : 'Nie udało się usunąć zadania.')
    }
  }
  ...
  const canManageTask = (task: TaskDto) => isAdmin || task.assigneeId === currentUser?.userId

  const modalAssigneeOptions = getProjectAssigneeOptions(form.projectId || selectedProjectId)
  ...
  const filteredTasks = useMemo(() => {
    ...
    return tasks.filter((task) => {
      ...
      if (filters.priority !== '' && task.priority !== Number(filters.priority)) {
        return false
      }
      ...
      if (filters.assignee && filters.assignee !== 'unassigned' && task.assigneeId !== filters.assignee) {
        return false
      }

      const dueDateValue = task.dueDate?.slice(0, 10)
      if (filters.dueFrom && (!dueDateValue || dueDateValue < filters.dueFrom)) {
        return false
      }
      ...
      <section className="workspace">
        <div className="card work wide-panel task-list-panel">
          <div className="task-list-header">
            <div>
              <h2>Lista zadań</h2>
              <span>{filteredTasks.length} z {tasks.length} zadań</span>
            </div>
          </div>
              ...
              <tbody>
                {filteredTasks.map((task) => (
                  <tr key={task.id}>
                    ...
                    <td>{task.assigneeId ? getUserName(task.assigneeId) : 'Nieprzypisane'}</td>
                    <td>
                    ...
                    <td>
                      {canManageTask(task) ? (
                        <div className="table-actions">
                          {selectedProjectId && (
                            <button type="button" className="btn btn-outline-secondary" onClick={() => openEditModal(task)}>
                              Edytuj
                            </button>
                          )}
                          <button type="button" className="btn btn-outline-danger" onClick={() => handleDelete(task)}>
                            Usuń
                          </button>
                        </div>
                      ) : (
        ...
        <div className="modal-backdrop" role="presentation" onClick={closeModal}>
          ...
          >
            <div className="modal-header">
              <h2 id="task-modal-title">{editingTask ? 'Edytuj zadanie' : 'Dodaj zadanie'}</h2>
